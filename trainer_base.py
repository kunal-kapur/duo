...
class TrainerBase(L.LightningModule):
  ...
  def forward(self, xt, sigma, bias=None, weight=None, pad_token_id=None):
    sigma = self._process_sigma(sigma)
    with torch.cuda.amp.autocast(dtype=torch.float32):
      model_output = self.backbone(xt, sigma, bias=bias, weight=weight, pad_token_id=pad_token_id)
